import { a as retainServed, c as BadRequest, i as planRoutes, l as errorMessage, n as SUFFIX_PROTOCOLS, o as routeProfile, r as mergeModels, s as soleRoute, t as OVERFLOW_SUFFIXES } from "./catalog-BV3bBZ_P.js";
...
const CODEX_SOURCE_LABEL = "Codex CLI";
/** The token file the Codex CLI maintains. */
function codexAuthPath() {
	const home = process.env["CODEX_HOME"];
	return join(home !== void 0 && home.length > 0 ? home : join(homedir(), ".codex"), "auth.json");
}
...
async function readCodexTokens() {
	...
	const access$1 = text(parsed.tokens?.access_token);
	const refresh = text(parsed.tokens?.refresh_token);
	return access$1 !== void 0 && refresh !== void 0 ? {
		access: access$1,
		refresh
	} : void 0;
}
/** Whether a Codex CLI login exists that this plugin could import. */
async function detectCodexLogin() {
	return await readCodexTokens() !== void 0;
}
/** One claim read out of a JWT payload without verifying it; undefined on any shape surprise. */
function jwtClaim(token, pick) {
	const part = token.split(".")[1];
	if (part === void 0) return void 0;
	try {
		return pick(JSON.parse(Buffer.from(part, "base64url").toString("utf8")));
	} catch {
		return;
	}
}
...
function isLinkedCodexCredential(credential) {
...
async function readCodexCredential() {
	...
	if (tokens === void 0) throw new BadRequest("dsh-providers: no Codex CLI login found to import (run `codex login` first)");
	const accountId = jwtClaim(tokens.access, (payload) => payload["https://api.openai.com/auth"]?.chatgpt_account_id);
	if (text(accountId) === void 0) throw new BadRequest("dsh-providers: the Codex CLI token carries no ChatGPT account id, so it cannot serve requests here");
	const exp = jwtClaim(tokens.access, (payload) => payload["exp"]);
	const expires = typeof exp === "number" && Number.isFinite(exp) ? exp * 1e3 - 300 * 1e3 : 0;
	return {
		type: "oauth",
...
*/
async function mirrorCodexTokens(credential) {
	...
	try {
		await withFileLock(path, async () => {
			...
			parsed["last_refresh"] = (/* @__PURE__ */ new Date()).toISOString();
...
*/
async function adoptCodexTokens(current) {
	try {
		const fromFile = await readCodexCredential();
		return fromFile.refresh !== current.refresh ? fromFile : void 0;
	} catch {
		return;
	}
}
...
var RemoteCatalog = class {
	...
	*/
	...
	async load() {
		...
		let text$1;
		try {
			text$1 = await readFile(this.path, "utf8");
		} catch (error) {
...
var AuthStore = class {
	...
	*/
	...
	async loadFresh() {
		...
		let text$1;
		try {
			text$1 = await readFile(this.authPath, "utf8");
		} catch (error) {
			if (error.code === "ENOENT") return {};
			throw error;
		}
		if (text$1.trim().length === 0) return {};
		const parsed = JSON.parse(text$1);
	...
	async modify(providerId, fn, _options) {
		return this.locked(async () => {
			...
			try {
				...
				const adopted = isLinkedCodexCredential(current) ? await adoptCodexTokens(current) : void 0;
				if (adopted === void 0) throw error;
				next = adopted;
			}
...
*/
function piAiVersionSkew() {
	const manifestOf = (name, specifier) => {
		try {
	...
	if (adapter !== void 0) for (const base of [join(adapter.dir, "node_modules"), dirname(dirname(adapter.dir))]) try {
		const nested = createRequire(import.meta.url)(join(base, "@earendil-works", "pi-ai", "package.json"));
		if (nested.name === "@earendil-works/pi-ai" && nested.version !== void 0) {
...
var AuthCredentialProvider = class extends LocalCredentialProvider {
	...
	*/
	...
	async providerView(provider, stored) {
		...
		if (provider.id === CODEX_PROVIDER_ID && credential === void 0 && await detectCodexLogin()) view.importSource = CODEX_SOURCE_LABEL;
		if (view.routed) {
	...
	*/
	async routeProvider(providerId, credential) {
		...
		await applyRouteOps(this.ctx, snapshotOps(this.ctx, providerId, ref, snapshot));
		throw new Error(`dsh-providers: the installed llm adapter cannot serve ${providerId}${declared === void 0 ? " — it speaks a wire protocol the adapter only reaches through its own catalog, which this dsh release does not carry" : " — even described in full"}, so its route was left as it was (${piAiVersionSkew()})`);
	}
	/**
	...
	*/
	async commitRoutes(commits) {
		...
		throw new Error(`dsh-providers: the installed llm adapter would not serve the updated routes, so the previous ones were put back (${piAiVersionSkew()})`);
	}
	/**
	...
	*/
	...
	async importCredential(providerId) {
		if (providerId !== CODEX_PROVIDER_ID) throw new BadRequest(`dsh-providers: no local login import exists for ${providerId}`);
